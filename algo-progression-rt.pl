...
=head1 SYNOPSIS
  ...
  perl algo-progression-rt.pl --verbose
  perl algo-progression-rt.pl --midi_port=synth --chord_patch=5 --genre=rock --arping
  perl algo-progression-rt.pl --midi_port=synth --chord_patch=4 --genre=rock --arping  --parts='Amv-Amc'
...
my %opt = (
    midi_port    => 'iac',   # REQUIRED MIDI device
    bpm          => 80,      # beats per minute
    genre        => '',      # a MIDI::Drummer::Tiny::Grooves category like 'rock'
    tonic        => 'A',     # tonic note for recreating new parts
    allowed      => 'minor', # allowed scale notes for recreating new parts
    parts        => 'DMv-AMv-Bmc-GMc', # top-level parts
    pairs        => 1,       # number of pairs of phrases rendered per round
    reps         => 1,       # number of times to repeat an individual phrase
    multi        => 1,       # number of times the phrases are repeated
    chord_patch  => 0,       # MIDI program number for the chords part
    bass_patch   => 35,      # MIDI program number for the bass part
    chord_octave => 5,       # octave of the chords part
    bass_octave  => 2,       # octave of the bass part
    divisions    => 4,       # number of divisions in this 4/4 composition
    channel      => 0,       # MIDI channel the chords part starts on
    arping       => 0,       # arpeggiating or not?
    verbose      => 0,
);
GetOptions(\%opt,
    'midi_port=s',
