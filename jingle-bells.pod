...
=for html
  <audio controls><source src="https://github.com/ology/Music/raw/master/jingle-bells-plain.mid" type="audio/mp3"></audio>
...
young guitarist elf pointed to L<CPAN|https://metacpan.org/> and the
...
Then after reading the examples, the team added the module and
modified the source and gave it a try... They listened to each of the
ornaments and decided to trigger them from a dispatch table, based for
the moment, on a loop counter. More sophisticated triggers based on
concepts of music theory were considered, but Christmastime was fast
approaching!
...
Here is the prototype code they came up with for Santa:
    ...
    use constant MAX => 16; # The number of notes before resetting the note counter
    ...
    # Dazzle with musical oranmentation (based on beat position for now)
    my %dazzle = (
    ...
    # Write out the fancy new score as a MIDI file
...
=for html
  <audio controls><source src="https://github.com/ology/Music/raw/master/jingle-bells-fancy.mid" type="audio/mp3"></audio>
...
=for :list
* L<The program|https://github.com/ology/Music/blob/master/jingle-bells>
* L<MIDI::Util>
