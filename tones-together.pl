...
my $timer = IO::Async::Timer::Periodic->new(
    ...
    on_tick  => sub {
        ...
        if ($ticks % $sixteenth == 0) {
            ...
            on(\@onsets, \$i, $beat_count, \@queue, 0);
            on(\@onsets2, \$i2, $beat_count, \@queue2, 1);
...
sub on ($ons, $inc, $count, $q, $chan) {
    my $selected;
    if (defined $ons->[$$inc] && $ons->[$$inc] == $count) {
        $selected = $q->[$$inc];
        say "$$inc, $count, ", ddc $selected;
        $midi_out->note_on(
            $chan,  # channel
            $selected->{pitch},
            127 # velocity
        );
        $$inc++; # increment the queue index
    }
    return $selected;
}
